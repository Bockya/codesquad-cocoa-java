...
public class MenuFile extends Frame {
    ...
    private class MyMenuBar {
        ...
        MyMenuBar(String title) {
            ...
            //편집 메뉴
            mEdit = new Menu("편집(E)");
            miUndo = new MenuItem("실행 취소(U)");
            miCut = new MenuItem("잘라내기(T)");
            miCopy = new MenuItem("복사(C)");
            miPaste = new MenuItem("붙여넣기(P)");
            miDelete = new MenuItem("삭제(L)");
            miSearchwithBeing = new MenuItem("Bing으로 검색(S)...");
            miFind = new MenuItem("찾기(F)...");
            miFindNext = new MenuItem("다음 찾기(N)");
            miFindPrevious = new MenuItem("이전 찾기(V)");
            miReplace = new MenuItem("바꾸기(R)...");
            miGoTo = new MenuItem("이동(G)...");
            miSelectAll = new MenuItem("모두 선택(A)");
            miTimeDate = new MenuItem("시간/날짜(D)");
        ...
        class EventHandler implements ActionListener {
            public void actionPerformed(ActionEvent e) {
                ...
                if (e.getSource() == miCopy) {
                    copiedText = content.getSelectedText(); //선택된 부분만 복사
